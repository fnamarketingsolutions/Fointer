...
const ACTION_MENU_EST_HEIGHT = 88;
...
export default function WatchGroupChatPage({
  ...
  const closeActionMenu = useCallback(() => {
    ...
    actionMenuButtonRef.current = null;
  }, []);

  const computeActionMenuPos = useCallback((buttonEl) => {
    if (!buttonEl) return null;
    const rect = buttonEl.getBoundingClientRect();
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    const spaceBelow = vh - rect.bottom - ACTION_MENU_GAP;
    const openAbove =
      spaceBelow < ACTION_MENU_EST_HEIGHT &&
      rect.top > ACTION_MENU_EST_HEIGHT + ACTION_MENU_GAP;
  ...
  const toggleActionMenu = useCallback(
    (messageId, buttonEl) => {
      ...
      actionMenuButtonRef.current = buttonEl;
      setActionMenuPos(computeActionMenuPos(buttonEl));
      setOpenActionMenuId(messageId);
    },
    [closeActionMenu, computeActionMenuPos, openActionMenuId]
  );
  ...
  useEffect(() => {
    ...
    const reposition = () => {
      ...
      setActionMenuPos(computeActionMenuPos(actionMenuButtonRef.current));
    };

    window.addEventListener("resize", reposition);
    // Reposition on scroll (capture) so menus stay anchored while the chat scrolls
    document.addEventListener("scroll", reposition, true);
    return () => {
      window.removeEventListener("resize", reposition);
      document.removeEventListener("scroll", reposition, true);
    };
  }, [openActionMenuId, computeActionMenuPos]);
  ...
  useEffect(() => {
    ...
    const onStatusUpdated = (payload) => {
      if (String(payload?.groupId) !== activeGroupId) return;
      const nextStatus = payload?.status || payload?.watchGroup?.status;
    ...
    socket.on(watchGroupSocketEvents.groupStatusUpdated, onStatusUpdated);
    ...
    return () => {
      ...
      socket.off(watchGroupSocketEvents.groupStatusUpdated, onStatusUpdated);
    };
  ...
  const handleSend = async () => {
    ...
    if (
      !editingMessageId &&
      (chatMeta?.isPaused || chatMeta?.group?.status === "paused")
    ) {
      setComposerError("This watch group is paused.");
      return;
    }
  ...
  const activeActionMessage = useMemo(
    () =>
      openActionMenuId
        ? messages.find((row) => String(row.id) === String(openActionMenuId)) ||
          null
        : null,
  ...
  const communityLabel =
    chatMeta?.group?.community?.name || "No community";
  ...
  const canSendMessages = Boolean(chatMeta?.myRole) && !isPaused;

  const toggleGroupPaused = async () => {
    if (!isGroupOwner || pauseLoading) return;
    const nextPaused = !isPaused;
    setPauseLoading(true);
    try {
      const data = await setWatchGroupPaused(groupId, nextPaused);
      const nextStatus =
        data?.watchGroup?.status || (nextPaused ? "paused" : "active");
      setChatMeta((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
      ...
      showToast(nextPaused ? "Group paused." : "Group resumed.");
    } catch (err) {
      showToast(getErrorMessage(err, "Failed to update group status."));
    } finally {
  ...
  return (
    <div className="h-full bg-[#0F0C09] overflow-hidden flex flex-col min-h-0">
      ...
      <div className="px-3 sm:px-4 py-3 border-b border-[#2A241E] bg-[#14100D] sticky top-0 z-20 shrink-0">
        <div className="flex items-center gap-3">
          ...
          {isGroupOwner ? (
            <button
              type="button"
              onClick={toggleGroupPaused}
              disabled={pauseLoading || loading}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[10px] font-semibold uppercase tracking-wide shrink-0 transition-colors disabled:opacity-60 ${
                isPaused
                  ? "bg-[#2A241E] text-[#D4AF37] hover:bg-[#342C24]"
                  : "bg-[#1D1713] text-[#8C8070] hover:text-[#E5E0D8] hover:bg-[#251E17]"
              }`}
            ...
            <span
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[10px] font-semibold uppercase tracking-wide shrink-0 ${
                isPaused
                  ? "bg-[#2A241E] text-[#D4AF37]"
                  : "bg-[#1D1713] text-[#8C8070]"
              }`}
              title={isPaused ? "Group is paused" : "Group is running"}
            >
          ...
          <>
            ...
            {messages.map((message) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative max-w-[86%] sm:max-w-[75%] rounded-2xl px-3 py-2 border ${
                    ...
                    {message.status === "deleted" ? (
                      ...
                      <>
                          ...
                          <div className="flex items-center justify-between gap-2 mb-1">
                            ...
                            {showActions ? (
                              <button
                                type="button"
                                onClick={(e) =>
                                  toggleActionMenu(message.id, e.currentTarget)
                                }
                                className="p-1 rounded-md shrink-0 hover:bg-black/20 text-black/60 hover:text-black"
                        ...
                        >
                          {message.text ? (
                            <p
                              className={`text-sm whitespace-pre-wrap break-words flex-1 min-w-0 ${
                                isMine
                                  ? "text-[#E5E0D8]"
                          ...
                          {isMine && showActions ? (
                            <button
                              ...
                              className="p-1 rounded-md shrink-0 self-end hover:bg-black/20 text-[#A69B8D] hover:text-[#E5E0D8]"
                              title="Message actions"
                            >
                              <EllipsisVertical size={14} />
                            </button>
      ...
      {openActionMenuId && actionMenuPos && activeActionMessage ? (
        <div
          ref={actionMenuRef}
          style={{
            position: "fixed",
          ...
          className="bg-[#14100D] border border-[#2A241E] rounded-lg shadow-xl p-1 z-[100]"
        >
          {activeActionMessage.canEdit ? (
            <button
              type="button"
              onClick={() => openEdit(activeActionMessage)}
          ...
          {activeActionMessage.canDelete ? (
            <button
              type="button"
              onClick={() => promptDelete(activeActionMessage.id)}
      ...
      <div className="border-t border-[#2A241E] bg-[#14100D] p-2 sm:p-3 space-y-2 shrink-0">
        {isPaused ? (
          <p className="text-xs text-[#D4AF37]/90 text-center py-1">
            This group is paused. Messaging is disabled until an admin resumes
            it.
          </p>
        ) : !canSendMessages ? (
