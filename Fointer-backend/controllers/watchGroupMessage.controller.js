...
const canDeleteMessage = (message, userId, group, canModerate) => {
...
const formatOpts = (access, userId) => ({
  isGroupCreator: isWatchGroupCreator(access.group, userId),
...
export const createWatchGroupMessage = async (req, res) => {
  try {
    ...
    const canModerate = await resolveCommunityModeration(access.group, req.user);
    const accessWithMod = { ...access, canModerate };
    ...
    const message = formatMessage(
      populated,
      req.user._id,
      formatOpts(accessWithMod, req.user._id)
    );
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Messages can only be deleted by the author within 60 minutes, the group creator, or a community moderator.",
        editWindowMinutes: MESSAGE_EDIT_WINDOW_MINUTES,
      });
    }
