...
export default function CommunityBrowseDetail({
  ...
  const panelClass = isPage
    ? ""
    : "bg-[#14100D] border border-[#2A241E] rounded-xl p-4";
  const statTileClass = isPage
    ? ""
    : "bg-[#0E0C0A] border border-[#2A241E] rounded-lg p-2 sm:p-3";
  ...
  const headerBlock = community && isPage && (
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        <h3 className="font-serif font-semibold text-[#D4AF37] text-2xl sm:text-3xl">
          {community.name}
        </h3>
...
<div className="w-200 h-200 bg-gradient-to-br from-[#1C1612] to-[#0E0C0A] flex items-center justify-center rounded-lg" />        )}
        {!isPage && (
          <>
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-5">
              ...
              <h3 className="font-serif font-semibold text-[#D4AF37] text-xl sm:text-2xl">
                {community.name}
              </h3>
            </div>
          </>
  ...
  const galleryStrip = community && thumbs.length > 0 && (
    <div>
  ...
  const statsBlock = community && (
    ...
    >
      <div className={statTileClass}>
        <div
          className={`font-serif text-[#D4AF37] ${
            isPage ? "text-base sm:text-lg" : "text-lg"
          }`}
        >
      ...
      <div className={statTileClass}>
        <div
          className={`font-medium text-[#E5E0D8] truncate ${
            isPage ? "text-xs sm:text-sm" : "text-sm"
          }`}
        >
          {ownerName}
        </div>
      ...
      <div className={statTileClass}>
        <div
          className={`font-medium text-[#E5E0D8] ${
            isPage ? "text-xs sm:text-sm" : "text-sm"
          }`}
        >
  ...
  const rulesBlock = community?.rules && (
    <section className={panelClass}>
        ...
        <>
          <ol className="space-y-2">
            {primaryRules.map((rule, idx) => (
              <RuleItem key={`${idx}-${rule.slice(0, 24)}`} index={idx} rule={rule} />
            ))}
          </ol>
          {extraRules.length > 0 && (
            <>
  ...
  const tagsBlock = community?.tags?.length > 0 && (
  ...
  const membersBlock = community?.isMember && isAuthenticated && (
    <section className={panelClass}>
      <div className="flex items-center gap-2 mb-3">
        <Users size={14} className="text-[#D4AF37]" />
        <h4 className="text-sm font-semibold text-[#E5E0D8]">Members</h4>
        <span className="text-xs text-[#8C8070]">({members.length})</span>
      </div>
      ...
      {membersLoading ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <Collapsible open={membersExpanded}>
            <div className="space-y-2 pb-1">
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="flex items-center gap-2.5 p-2 rounded-lg bg-[#0E0C0A] border border-[#2A241E]"
                  >
                    <MemberAvatar user={user} size="sm" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-xs font-medium text-[#E5E0D8] truncate">
                          {displayName}
                        </span>
                        {user.username && (
  ...
  const postsSection = community?.isMember && isAuthenticated && (
    <section>
  ...
  const communityBody = community && (
    <>
      {isPage ? (
        <>
          {headerBlock}
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-4 lg:gap-5">
            <div className="lg:self-start">{heroBlock}</div>
            <div className="flex flex-col divide-y divide-[#2A241E]/60">
              {sidebarBlocks.map(([key, node]) => (
