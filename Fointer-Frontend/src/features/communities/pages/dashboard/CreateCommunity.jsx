...
import { makeGalleryItem } from "../../../../shared/utils/media";
import { MAX_GALLERY_IMAGES, MAX_FILE_SIZE } from "../../../../shared/constants/uploads";
...
const TYPE_OPTIONS = [
  ...
  },
  {
    ...
    description: "Hidden from search. Owner invites members; they accept or decline.",
    icon: ShieldCheck,
  },
  {
    value: "private_request",
    label: "Private-Request",
    description: "Discoverable. Members request access or owners send invites — either side accepts.",
    icon: Lock,
  },
];

const typeLabel = formatCommunityType;
...
export default function CreateCommunity({ onCreated }) {
  ...
  const handleCoverChange = (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError("Cover image size should be under 5MB.");
      return;
    }
    setCoverFile(file);
  ...
  const handleGalleryChange = (e) => {
    ...
    if (files.length > remaining) {
      setError(
        `Only ${remaining} more gallery image${remaining === 1 ? "" : "s"} can be added (max ${MAX_GALLERY_IMAGES}).`
      );
    } else {
      setError("");
    }
        ...
        >
          <div className="space-y-3">
            ...
            <div
              ...
              className="relative w-full h-[400px] sm:h-52 rounded-xl bg-[#0E0C0A] border border-dashed border-[#2A241E] hover:border-[#D4AF37]/50 overflow-hidden flex items-center justify-center cursor-pointer transition-all group"
            >
              {displayCover ? (
                <>
                          <img
              ...
              />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity pointer-events-none sm:pointer-events-auto">
                    <span className="text-xs text-white bg-black/60 px-3 py-1.5 rounded-md border border-white/20 items-center gap-1.5 hidden sm:inline-flex">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 z-10 p-1.5 rounded-md bg-black/70 text-red-400 border border-red-500/30 hover:bg-red-500/20"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {galleryItems.map((item) => (
                <div
                  key={item.id}
                  className="relative aspect-[4/3] rounded-lg overflow-hidden border border-[#2A241E] bg-[#0E0C0A] group"
                >
                  <img
                    src={item.preview}
                    alt="Gallery"
                  ...
                  <button
                    ...
                    className="absolute top-1.5 right-1.5 z-10 p-1 rounded-md bg-black/70 text-red-400 border border-red-500/30 hover:bg-red-500/20"
              ...
              {galleryItems.length < MAX_GALLERY_IMAGES && (
                <button
                  ...
                  className="aspect-[4/3] rounded-lg border border-dashed border-[#2A241E] hover:border-[#D4AF37]/50 bg-[#0E0C0A] flex flex-col items-center justify-center gap-1.5 text-[#8C8070] transition-colors"
                >
                  <ImagePlus size={20} className="text-[#D4AF37]" />
                  <span className="text-[10px] sm:text-xs">Add images</span>
                </button>
              )}
            </div>

            <p className="text-[10px] text-[#8C8070]">
              Optional. Select multiple images at once. PNG, JPG, WEBP up to 5MB each.
            </p>
          </div>
